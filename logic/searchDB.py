import logging
from logic.drinksSqlDb import query_drink_contains, get_formatted_ingredients, get_drink_session, close_session, ing_contains_all
...
def drink_search(drink_list):
    ...
    log.debug("drink_search received search term {}".format(drink_list))
    ...
    if num_drinks == 0:
        log.debug("No drinks sent for search, exiting drink_search()")
    ...
    for index in range(num_drinks):
        ...
        log.debug("Recipe returned from query_drink_contains is {}".format(recipe))
